...
for (file_index in 1:length(files)) {
 ...
 # p <- ppp(x,y,c(1,max(x)),c(1,max(y)))
  ...
  res <- optimise(discrep, interval = c(0, 50), maximum = TRUE)
  print(res)
  ...
  #fitp <- profilepl(D, Geyer, p ~ marks, aic=FALSE)
...
names(df) <-  c("Name","Intercept","x","y","I(x^2)","I(x * y)","I(y^2)","log(Interaction)","gamma","OptL","LObj","Interaction Distance", "Saturation Parameter")
df

# Write CSV in Desktop
write.csv(df, file = "~/index copy/_book/PhD_code/Spatial project/Data/BCA/BCA_Mp2p6n_Geyer_modified.csv")
